...
func _on_PlayButton_pressed():
	...
	$Tween.interpolate_property($Camera2D, "position", $Camera2D.position, Vector2($Camera2D.position.x, get_viewport().get_size().y/2 + 51), tweenDuration, Tween.TRANS_QUAD, Tween.EASE_OUT)
